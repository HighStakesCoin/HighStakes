...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered highstakes address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 HISC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal highstakes:URI (highstakes:XyZ...?message=example)
            {
...
static void NotifyzHISCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzHISCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzHISCReset.connect(boost::bind(NotifyzHISCReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzHISCReset.disconnect(boost::bind(NotifyzHISCReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zhiscTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
