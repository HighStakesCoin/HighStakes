...
#include "ui_zhisccontroldialog.h"
...
std::set<std::string> ZHiscControlDialog::setSelectedMints;
std::set<CMintMeta> ZHiscControlDialog::setMints;

ZHiscControlDialog::ZHiscControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZHiscControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZHiscControlDialog::~ZHiscControlDialog()
{
    delete ui;
}

void ZHiscControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHiscControlDialog::updateList()
{
...
void ZHiscControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHiscControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZHisc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHiscControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZHiscControlDialog::GetSelectedMints()
{
...
void ZHiscControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
