...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::HISC);
    ...
    if (!settings.contains("nAnonymizeHighStakesAmount"))
        settings.setValue("nAnonymizeHighStakesAmount", 1000);

    nAnonymizeHighStakesAmount = settings.value("nAnonymizeHighStakesAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeHighStakesAmount"))
        SoftSetArg("-anonymizehighstakesamount", settings.value("nAnonymizeHighStakesAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in highstakes.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHighStakesAmount:
            return QVariant(nAnonymizeHighStakesAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHighStakesAmount:
            nAnonymizeHighStakesAmount = value.toInt();
            settings.setValue("nAnonymizeHighStakesAmount", nAnonymizeHighStakesAmount);
            emit anonymizeHighStakesAmountChanged(nAnonymizeHighStakesAmount);
            break;
        case CoinControlFeatures:
