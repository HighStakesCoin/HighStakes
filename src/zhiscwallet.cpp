...
CzHISCWallet::CzHISCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zhisc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZHISCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZHISCSeed_deprecated()) {
                    LogPrintf("%s: Updated zHISC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zhisc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zhisc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzHISCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZHISCCount(nCountLastUsed);
    else if (!walletdb.ReadZHISCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzHISCWallet::Lock()
{
    seedMaster = 0;
}

void CzHISCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzHISCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZHISC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzHISCWallet::LoadMintPoolFromDB()
{
...
void CzHISCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzHISCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzHISCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zhiscTracker->HasPubcoinHash(pMint.first)) {
...
bool CzHISCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZHISC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zhiscTracker which also adds to database
    pwalletMain->zhiscTracker->Add(dMint, true);
...
void CzHISCWallet::SeedToZHISC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzHISCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzHISCWallet::GenerateDeterministicZHISC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzHISCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzHISCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
