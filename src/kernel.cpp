...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZHiscStake(spend));
    } else {
        ...
        CHiscStake* hiscInput = new CHiscStake();
        hiscInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(hiscInput);
    }
