...
// Copyright (c) 2016-2018 The HighStakes developers
...
public:
    ...
    bool WriteZHISCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZHISCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZHISCSeed_deprecated(uint256& seed);
    bool EraseZHISCSeed();
    bool EraseZHISCSeed_deprecated();

    bool WriteZHISCCount(const uint32_t& nCount);
    bool ReadZHISCCount(uint32_t& nCount);
